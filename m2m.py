...
anal = doe_analysis.analysis('GEN1')
...
## #                      {'THICK': 0.5, 'TEMP': 0.0, 'PRES': self.Pressure } )
    ...
    pp['000'] = point(0.0, -0.25, 0.0, common_params)
    pp['100'] = point(8.0, -0.25, 0.0, common_params)
    pp['010'] = point(0.0, 0.25, 0.0, common_params)
    pp['110'] = point(8.0, 0.25, 0.0, common_params)
    



    pp['001'] = point(0.0, -0.25, 8.0, common_params)
    pp['011'] = point(0.0, 0.25, 8.0, common_params)
    ...
    ## ms[mname] = self.add_block(mname)
    ## ms[mname].set_material(mat)
    ## ms[mname].define_block_from_mid_plane_points(pp['000'], pp['001'], pp['011'], pp['010'] )
    ...
    #self.add_block_face_hinge_join(ms['A'], FTAGS.MAX_U, DIR.V,
    #                               ms['B'], FTAGS.MIN_U, DIR.V) 
    ...
    gl = ms['A'].get_grid_list_from_tags(GTAGS.MIN_U)
    ## gl = gl + ms['A'].get_grid_list_from_tags(GTAGS.MAX_U)
    for g in gl :
         g.bc.add(DOF.DX)
         g.bc.add(DOF.DY)
         g.bc.add(DOF.DZ)
