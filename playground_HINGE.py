...
anal = doe_analysis.analysis('PLAYGROUND_HINGE')
...
##        -1,-1,-1                 1,-1,-1
    ...
    gla = blk_a.get_grid_list_from_tags(GTAGS.MAX_U)
    gla = blk_a.get_grid_list_from_tags(GTAGS.CENTER_V, gla)
    gla = blk_a.get_grid_list_from_tags(GTAGS.MAX_W, gla)

    # KEEP BLOCK A IN THE PLANE
    print 'BLOCK A POINTS...'
    for g in gla :
        print g.id
        g.bc.add(DOF.DY)
        g.bc.add(DOF.DZ)
